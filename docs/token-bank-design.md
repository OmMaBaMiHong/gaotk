...
日期：2026-09-25。状态：用户已要求实施，直接在原目录 main 开发；不部署。

文档现址：/Users/wade/work-space/sub2api/docs/token-bank-design.md。旧工作树链接已失效。

本轮采用原有实际计费金额为基数，出租者 80%、平台 20%，自动进入站内余额；配置明确的 Admin 收款用户。金额为零不分账，订阅用量按原计费类型单独标记，不标为现金实收。页面采用本方案既定的概览卡、账号列表和流水布局，复用现有设计系统。
...
- 已进入功能实现阶段；生产账号、分组、价格和余额仅在后续部署并实际启用时发生变化。本轮不部署。

实施口径：按现有实际计费金额分账，进入站内余额；独立结算价、提现不纳入此次实现。
...
实施口径：继续用现有渠道/模型计价及倍率计算调用者实际账单。以统一量化后的 BalanceCost 或 SubscriptionCost 为基数 B，原 BillingType 随流水保留：
...
订阅额度消耗按其原实际计费金额分配站内余额，并在流水明确标为订阅额度，不标为现金实收。实际金额为零时不分账；非零赠送余额、自用命中及已有透支记账沿用同一分配规则。AccountStatsCost 不作为结算依据。平台应按这一起算口径配置允许进入出租池的消费分组。
...
1. 上游整合：已完成，全部旧开发代码归并并推送 gaotk/main。此后直接在原目录 main 开发，不另建分支。
...
- `backend/internal/service/token_bank_strategy.go`：`RevenueShareStrategy` 接口、`ProportionalRevenueShare` 比例策略、`RentalRevenueStrategy` 模板选择入口。只分配已有金额，不读写数据库、不计算 Token 价格。
- `backend/internal/repository/token_bank_settlement.go`：独立事务参与者。准备阶段按用户 ID 顺序锁定消费者、出租者和 Admin；执行阶段写独立账本并原子加余额。
- `backend/internal/service/token_bank_cache.go`：提交后刷新所有参与者的余额及认证缓存。
- 原 `UsageBillingRepository.Apply` 仅增加准备和结算两个调用。原定价、倍率、消费者扣费、额度逻辑保持不变；自营账号无需额外分账查询。
- 出租快照随实际被调度账号进入计费命令，纳入幂等指纹；影子账号继承凭据母账号归属。更改比例或暂停不会重算历史/在途请求。
- 不通过扫描 usage_logs 加余额；账本独立于日志保留周期。原幂等记录与账本唯一键共同防重复。

## 11. 当前交付范围与使用

1. C 端「Token 银行」：OpenAI、Claude、DeepSeek；多个账号、API Key 验证导入、OpenAI/Claude 官方 OAuth、同账号重新授权、暂停/恢复、平台与出租状态筛选、调用量、Token 量、累计/今日收益、分页收益流水。
2. 管理端「Token 银行」：配置各平台同平台出租分组与收款 Admin（默认二八分成），查看账号所属用户和平台汇总，按用户、平台、状态筛选。现有账号管理列表新增所属出租用户链接。
3. 凭据和分组由服务端控制。OAuth 会话绑定用户、平台、state、10 分钟有效期及单次使用。重新授权必须同一上游身份。API Key 在固定官方 `/models` 端点验证，拒绝重定向。
4. API Key 指纹只能识别同一个 Key，不宣称能识别同一真实账号下的不同 Key。OAuth 使用平台返回的账号身份。归属不可变，也不开放更换凭据为另一个上游账号。
5. 迁移 `242_token_bank.sql` 默认不创建/启用任何出租策略。管理员需先建立同平台分组，再在 Token 银行配置收款 Admin 并启用；消费渠道/组合路由必须关联对应出租组才会有流量。
6. 收益是站内余额，按系统账本 USD 单位显示；今日按北京时间统计。没有独立结算单价、保底收益、提现、借贷或利息功能。冲正是后续能力，本次不提供修改/删除收益流水的接口。
7. 不部署、不运行生产迁移，不动旧工作区中的本地模拟服务。

验证记录见 `docs/token-bank-verification.md`。
