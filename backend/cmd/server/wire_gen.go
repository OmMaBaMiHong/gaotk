...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	tokenBankRepository := repository.NewTokenBankRepository(db)
	...
	tokenBankService := service.NewTokenBankService(tokenBankRepository, accountRepository, openAIOAuthService, oAuthService, redisClient)
	tokenBankHandler := handler.NewTokenBankHandler(tokenBankService)
	userRepository := repository.NewUserRepository(client, db)
	redeemCodeRepository := repository.NewRedeemCodeRepository(client)
	refreshTokenCache := repository.NewRefreshTokenCache(redisClient)
	settingRepository := repository.NewSettingRepository(client)
	groupRepository := repository.NewGroupRepository(client, db)
	oAuthClientAppRepository := repository.NewOAuthClientAppRepository(client)
	settingService := service.ProvideSettingService(settingRepository, groupRepository, proxyRepository, configConfig, oAuthClientAppRepository)
	...
	oAuthClientAppService := service.NewOAuthClientAppService(oAuthClientAppRepository, configConfig, settingService)
	authHandler := handler.NewAuthHandler(configConfig, authService, userService, settingService, promoService, redeemService, totpService, userAttributeService, oAuthClientAppService)
	...
	oAuthClientAppHandler := admin.NewOAuthClientAppHandler(oAuthClientAppService)
	upstreamBillingProbeService := service.ProvideUpstreamBillingProbeService(accountRepository, accountTestService, settingService, leaderLockCache, db)
	openCodeGoUsageService := service.ProvideOpenCodeGoUsageService(accountRepository, httpUpstream, settingService, leaderLockCache, db)
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, accountHandler, adminAnnouncementHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, kimiOAuthHandler, antigravityOAuthHandler, grokOAuthHandler, cnProviderHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, pluginHandler, adminAPIKeyHandler, scheduledTestHandler, channelHandler, channelMonitorHandler, channelMonitorRequestTemplateHandler, contentModerationHandler, promptAdminHandler, paymentHandler, affiliateHandler, complianceHandler, auditLogHandler, oAuthClientAppHandler, upstreamBillingProbeService, ollamaCloudUsageService, openCodeGoUsageService)
