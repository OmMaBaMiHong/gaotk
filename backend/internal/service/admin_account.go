...
func (s *adminServiceImpl) UpdateAccount(ctx context.Context, id int64, input *UpdateAccountInput) (*Account, error) {
	...
	if account.IsCredentialShadow() && input.Credentials != nil {
		...
		if OwnedAccountUserID(ctx) > 0 && !isTokenSavingsRefresh(ctx) {
			// The owner scope already built the full credential map and verified
			// replacements. Do not restore the previous identity's removed secrets.
