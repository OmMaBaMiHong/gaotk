...
func NewOAuthClientAppService(repo OAuthClientAppRepository, cfg *config.Config, settings *SettingService) *OAuthClientAppService {
	s := &OAuthClientAppService{repo: repo, legacy: cfg.OAuthServer, settings: settings}
...
func (s *OAuthClientAppService) ensureLegacySeeded() {
	...
	if s.settings != nil {
		saved, settingsErr := s.settings.GetOAuthServerSettings(ctx)
		if settingsErr != nil {
			slog.Warn("oauth client legacy settings unavailable", "error", settingsErr)
			return
		}
		redirectURIs = strings.Join(saved.RedirectURIs, "\n")
	}
