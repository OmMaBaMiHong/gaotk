...
describe('Token Savings receiving rules editor', () => {
  it('never infers a plan from a group name and requires explicit OpenAI eligibility', async () => {
    ...
    expect(editor.props('rules')).toEqual([{ group_id: 3, priority: 0, allowed_plans: [] }])
    ...
    expect(selector.findAll('option').map(option => [option.attributes('value'), option.text()])).toEqual([
      ['pro', 'Pro20x'], ['prolite', 'Pro5x'], ['plus', 'Plus'], ['free', 'Free'], ['team', 'Business Standard'], ['selfservebusinessprolite', 'Business Premium']
    ])
    await selector.setValue(['pro'])
    expect(editor.props('rules')).toEqual([{ group_id: 3, priority: 0, allowed_plans: ['pro'] }])
    view.unmount()
  })
  it('roundtrips every accepted plan and changes only the edited group', async () => {
    const view = render([3, 4], [
      { group_id: 3, priority: 800, allowed_plans: ['pro', 'team'] },
      { group_id: 4, priority: 100, allowed_plans: ['plus', 'free', 'prolite'] }
    ])
    const selector = view.get<HTMLSelectElement>('[data-testid="receiving-plans-3"]')
    ...
    expect(view.getComponent(TokenSavingsRulesEditor).props('rules')).toEqual([
      { group_id: 3, priority: 900, allowed_plans: ['pro', 'team'] },
      { group_id: 4, priority: 100, allowed_plans: ['plus', 'free', 'prolite'] }
    ])
    view.unmount()
  })
  it('removes unchecked rules and keeps non-OpenAI plan restrictions empty', async () => {
    const view = render([3], [{ group_id: 3, priority: 200, allowed_plans: ['pro'] }])
    await view.get('[data-testid="receiving-group-5"]').setValue(true)
    expect(view.find('[data-testid="receiving-plans-5"]').exists()).toBe(false)
    ...
    expect(view.getComponent(TokenSavingsRulesEditor).props('rules')).toEqual([{ group_id: 5, priority: 0, allowed_plans: [] }])
    view.unmount()
  })
})
...
describe('Token Savings save payload', () => {
  it('filters rules for removed/unlinked groups and clones allowed lists without losing multi-selection', () => {
    const rules = [
      { group_id: 3, priority: 800, allowed_plans: ['pro', 'team'] },
      { group_id: 4, priority: 100, allowed_plans: ['plus'] },
      { group_id: 5, priority: 40, allowed_plans: ['pro'] }
    ]
    const payload = buildReceivingConfig([3, 4, 5], rules, [groups[0], groups[2]])
    expect(payload).toEqual({ receiving_group_ids: [3, 5], receiving_rules: [rules[0], { group_id: 5, priority: 40, allowed_plans: [] }] })
    payload.receiving_rules[0].allowed_plans.push('free')
    expect(rules[0].allowed_plans).toEqual(['pro', 'team'])
    expect(buildReceivingConfig([5], rules, groups).receiving_rules).toEqual([{ group_id: 5, priority: 40, allowed_plans: [] }])
  })
  it('leaves legacy OpenAI groups unconfigured until the admin selects accepted plans', () => {
    ...
    expect(payload.receiving_rules).toEqual([
      { group_id: 3, priority: 0, allowed_plans: [] }, { group_id: 5, priority: 0, allowed_plans: [] }
    ])
  ...
  it('accepts boundary priorities and exact canonical plans only', () => {
    expect(validReceivingRules([{ group_id: 3, priority: 1000, allowed_plans: ['pro'] }, { group_id: 5, priority: 0, allowed_plans: [] }], groups)).toBe(true)
