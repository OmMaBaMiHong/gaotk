...
import { ref } from 'vue'
import TokenBankSetting from '../TokenBankSetting.vue'
...
const api = vi.hoisted(() => ({ config: vi.fn(), updateConfig: vi.fn() }))
...
const render = () => (wrapper = mount(TokenBankSetting, {}))

describe('Token Bank global setting', () => {
  it('defaults off and persists independently with the server response', async () => {
    api.config.mockResolvedValue({ enabled: false })
    api.updateConfig.mockResolvedValue({ enabled: true })
  ...
  it('disables mutation when initial read fails and allows retry', async () => {
    api.config.mockRejectedValueOnce(new Error('failed')).mockResolvedValue({ enabled: true })
    const view = render(); await flushPromises()
