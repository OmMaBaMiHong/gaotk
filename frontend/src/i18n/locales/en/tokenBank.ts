export default {
  unsupportedAccount: 'This platform or account type is not currently accepted. Please select again.',
  disabled: 'Token Bank is currently disabled.',
  apiKeyAccount: 'API Key (optional strategy)',
  officialSubscription: 'Official subscription OAuth',
  receivingAccountTypes: 'Accepted account types',
  settingImmediate: 'Saved independently and effective immediately.',
  settingEnabled: 'Enable Token Bank',
  settingHint: 'Controls user access and account enrollment. Disabled by default; configure channel receiving rules before enabling. Existing revenue records are retained.',
  settingTitle: 'Token Bank',
  showcaseLeaderboard: 'Earnings board',
  showcaseLeaderboardHint: 'Top 10 users by all-time credited savings earnings. Names are masked; earnings are credited to site balances.',
  showcaseEmpty: 'No credited earnings yet',
  showcaseBroadcast: '{name} earned {amount} from {platform} savings',
  showcaseRank: 'Rank',
  showcaseUser: 'User',
  showcaseUpdated: 'Updated {time}',
  showcasePause: 'Pause earnings broadcast',
  showcaseResume: 'Resume earnings broadcast',
  showcaseSettingTitle: 'Token Bank earnings showcase',
  showcaseSettingHint: 'Show real earnings with masked names and an all-time leaderboard at the top of Token Bank. Off by default; disabled hides the entire showcase.',
  showcaseSettingEnabled: 'Show earnings broadcast and leaderboard',
  showcaseSettingImmediate: 'This switch saves independently and takes effect immediately.',
  accountImportHint: 'Upload exported JSON files to import accounts in bulk. Imported accounts belong to your current user.',
  accountImportWarning: 'Accounts automatically join the receiving groups configured by administrators for each platform. Proxies are not imported.',

  invalidSavingsConfig: 'Enter a valid revenue share, receiving admin and at least one channel receiving group.',
  usageRecords: 'Usage records',
  deleteConfirm: 'Delete account “{name}”? Existing savings revenue records will be retained.',
  channelSavings: 'Token savings',
  channelSavingsHint: 'Set one revenue share for this channel and choose the groups that receive each platform’s accounts. Authorized accounts join matching platform groups automatically.',
  ownerShare: 'Owner share',
  receivingGroups: 'Receiving groups',
  receivingRulesHint: 'Accounts enter one eligible group with the highest priority; ties use the lowest group ID. OAuth plans are verified by the provider, never inferred from group names. API Key acceptance must be enabled separately; uncheck all account types to disable a rule.',
  receivingPriority: 'Receiving priority (0–1000)',
  receivingAllowedPlans: 'Accepted official plans (required, multiple selections)',
  receivingAllowedPlansHint: 'Hold Ctrl / Command to select multiple. GPT Pro20x, Pro5x and Plus are separate; Claude Pro and Max are separate. Eligibility is verified with the provider.',
  invalidReceivingRules: 'Set an integer priority from 0 to 1000 and valid account types for each group, and select paid plans for official subscription OAuth. Uncheck all types to disable a rule.',
  selectChannelGroupsFirst: 'Associate groups in the channel platform settings first.',

  allStatuses: 'All savings statuses',
  title: 'Token Bank',
  description: 'Share unused quota from your subscribed official GPT / Claude account and earn internal balance from actual usage. Earnings are not guaranteed and cannot be withdrawn as cash.',
  adminDescription:
    'Configure platform savings pools and the receiving admin. Review accounts and revenue shares; consumer pricing remains unchanged.',
  add: 'Add account',
  retry: 'Retry',
  today: 'Today (Beijing time)',
  total: 'Total earnings',
  accounts: 'Savings accounts',
  platformRevenue: 'Platform share',
  policies: 'Platform savings policies',
  policyHint:
    'Create a matching platform group first. Owners receive 80%, the platform 20%. The recipient must be an active admin. Route a consumer channel or composite route to this group to send traffic.',
  pool: 'Savings group',
  selectPool: 'Select matching group',
  adminRecipient: 'Receiving admin user ID',
  ratio: 'Owner {rate}%',
  enabled: 'Accept savingss',
  save: 'Save policy',
  platform: 'Platform',
  allPlatforms: 'All platforms',
  ownerID: 'Owner user ID',
  filter: 'Filter',
  loading: 'Loading…',
  closed:
    'Savingss are not yet enabled. An administrator must configure a platform.',
  empty:
    'No savings accounts yet. Add an authorized account to see its usage here.',
  account: 'Account',
  status: 'Savings status',
  usage: 'Usage',
  actions: 'Actions',
  paused: 'Paused',
  active: 'Available',
  unavailable: 'Unavailable',
  requests: 'calls',
  details: 'Earnings',
  resume: 'Resume',
  pause: 'Pause',
  reauthorize: 'Reauthorize',
  previous: 'Previous',
  next: 'Next',
  ledger: 'Revenue ledger',
  allAccounts: 'All accounts',
  ledgerHint:
    'Each call splits its existing billed amount and credits balances. Subscription amounts are quota consumption, not cash received for this call. Zero amounts earn nothing. Usage follows log retention; the revenue ledger is retained separately.',
  timeModel: 'Time / model',
  billed: 'Billed amount',
  ownerRevenue: 'Owner earnings',
  subscription: 'Subscription quota',
  balance: 'Balance billing',
  noRevenue: 'No revenue yet. Entries appear after your accounts serve calls.',
  name: 'Account name',
  method: 'Authorization method',
  openAuthorization: 'Open official authorization',
  callback: 'Full callback URL or authorization code',
  state: 'Authorization state',
  callbackHint:
    'Paste the full callback URL, or Claude code#state. For a code alone, retain the state generated for this session.',
  importHint:
    'Official credentials are verified. Accounts can only join their platform pool; credentials are never listed. Duplicate API keys are rejected, but distinct keys cannot prove a shared real account identity.',
  processing: 'Verifying…',
  verifySave: 'Verify and save',
  startAuthorization: 'Generate authorization link',
  failed: 'Operation failed. Please retry.',
  saved: 'Saved',
  ownerLabel: 'Savings owner #{id}',
  sourceSelf: 'Self-operated'
}
